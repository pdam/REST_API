...
class  snmp_community(object):
    ...
    def create(self ):
            self.cmd ="cli --quiet -c  \" snmp-community-create   \" \n"%()
    ...
    def show(self ):
        self.cmd ="cli --quiet -c  \" snmp-community-show  layout  horizontal parsable-delim DELIM show-headers \" \n"
        stdin, stdout, stderr = self.sshclient.exec_command(self.cmd)
        lines = stdout.readlines()
        assert len(lines) > 0
    ...
    def delete(self,**kwargs ):
        ...
        self.cmd ="cli --quiet -c  \" snmp-community-delete  %s \" \n"% (self.command)
        stdin, stdout, stderr = self.sshclient.exec_command(self.cmd)
        lines=stdout.readlines() 
        return  lines
