...
class  igmp_static_group(object):
    ...
    def create(self,  ):
            self.cmd ="cli --quiet -c  \" igmp-static-group-create   \" \n"%()
    ...
    def show(self ):
        self.cmd ="cli --quiet -c  \" igmp-static-group-show  layout  horizontal parsable-delim DELIM show-headers \" \n"
        stdin, stdout, stderr = self.sshclient.exec_command(self.cmd)
        lines = stdout.readlines()
        assert len(lines) > 0
    ...
    def delete(self,**kwargs ):
        ...
        self.cmd ="cli --quiet -c  \" igmp-static-group-delete  %s \" \n"% (self.command)
        stdin, stdout, stderr = self.sshclient.exec_command(self.cmd)
        lines=stdout.readlines() 
        return  lines
    ...
    def modify(self, **kwargs ):
        ...
        self.cmd ="cli --quiet -c  \" igmp-static-group-modify  %s  \" \n"% (self.command)
        stdin, stdout, stderr = self.sshclient.exec_command(self.cmd)
        lines=stdout.readlines() 
        return  lines
