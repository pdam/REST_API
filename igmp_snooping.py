...
class  igmp_snooping(object):
    ...
    def create(self, switch ,enable  ):
            self.cmd ="cli --quiet -c  \" igmp-snooping-create   switch %s   enable %s  \" \n"%(switch ,enable )
    ...
    def show(self ):
        self.cmd ="cli --quiet -c  \" igmp-snooping-show  layout  horizontal parsable-delim DELIM show-headers \" \n"
        stdin, stdout, stderr = self.sshclient.exec_command(self.cmd)
        lines = stdout.readlines()
        assert len(lines) > 0
    ...
    def delete(self,**kwargs ):
        ...
        self.cmd ="cli --quiet -c  \" igmp-snooping-delete  %s \" \n"% (self.command)
        stdin, stdout, stderr = self.sshclient.exec_command(self.cmd)
        lines=stdout.readlines() 
        return  lines
    ...
    def modify(self, **kwargs ):
        ...
        self.cmd ="cli --quiet -c  \" igmp-snooping-modify  %s  \" \n"% (self.command)
        stdin, stdout, stderr = self.sshclient.exec_command(self.cmd)
        lines=stdout.readlines() 
        return  lines
