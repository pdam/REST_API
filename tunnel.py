...
class  tunnel(object):
    ...
    def create(self,  ):
            self.cmd ="cli --quiet -c  \" tunnel-create   \" \n"%()
    ...
    def show(self ):
        self.cmd ="cli --quiet -c  \" tunnel-show  layout  horizontal parsable-delim DELIM show-headers \" \n"
        stdin, stdout, stderr = self.sshclient.exec_command(self.cmd)
        lines = stdout.readlines()
    ...
    def delete(self,**kwargs ):
        ...
        self.cmd ="cli --quiet -c  \" tunnel-delete  %s \" \n"% (self.command)
        stdin, stdout, stderr = self.sshclient.exec_command(self.cmd)
        lines=stdout.readlines() 
        return  lines
    ...
    def modify(self, **kwargs ):
        ...
        self.cmd ="cli --quiet -c  \" tunnel-modify  %s  \" \n"% (self.command)
        stdin, stdout, stderr = self.sshclient.exec_command(self.cmd)
        lines=stdout.readlines() 
        return  lines
